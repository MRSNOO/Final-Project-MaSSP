...
class FaceRecognizer:
  ...
  def __init__ (self):
    self.clf = None
    fn = config.data_dir + "train_data.csv"
    df = pd.read_csv(fn)
    self.X_train = []
    self.y_train = []
    for i in range(df.shape[0]):
      img = np.array(df.iloc[i])
      self.X_train.append(img[1:])
      self.y_train.append(img[0])

    # Detector = FaceRecognizer()
    self.init_KNN(self.X_train, self.y_train)

    # for image in ["frame3.jpg"]:
  ...
  def predict_and_encode (self, img, confidence = 0.995,ratio = 30,visua = False):
